...
function move(dir) {
    ...
    for(let i = 0 ; i < SIZE; i++) {
        ...
        for(let j = 0; j < SIZE; j++) {
            switch(dir) {
                case "LEFT": tileSet.push(Grid[i][j]); break;
                case "RIGHT": tileSet.push(Grid[i][SIZE - j - 1]); break;
                case "UP": tileSet.push(Grid[j][i]); break;
                case "DOWN": tileSet.push(Grid[SIZE - j - 1][i]); break;
                default: break;
            }
        }
